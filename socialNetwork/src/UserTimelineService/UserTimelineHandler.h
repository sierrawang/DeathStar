...
public:
  UserTimelineHandler(
      ...
      ClientPool<ThriftClient<UserTimelineStorageServiceClient>> *);
...
UserTimelineHandler::UserTimelineHandler(
    ...
    ClientPool<ThriftClient<UserTimelineStorageServiceClient>> *user_timeline_client_pool) {
...
void UserTimelineHandler::WriteUserTimeline(
  ...
  // This is all for updating a user in the database with the new post id
  ...
  if (!user_timeline_client_wrapper) {
    ...
    se.message = "Failed to connect to user-timeline-storage-service";
    throw se;
  }
  auto user_timeline_client = user_timeline_client_wrapper->GetClient();
  try {
    user_timeline_client->UpdateUserTimeline(user_id, post_id, timestamp);
  } catch (...) {
    _user_timeline_client_pool->Remove(user_timeline_client_wrapper);
    LOG(error) << "Failed to update user in user-timeline-storage-service";
    throw;
  }
...
void UserTimelineHandler::ReadUserTimeline(
  ...
  // Get all the post ids for this user
  // std::vector<std::string> post_ids_str;
  // try {
  //   if (_redis_client_pool)
  //     // Grab the posts from start stop in most recent order of being posted
  //     // For ex, start = 0, stop = 3 means grab the three most recent posts
  ...
  // // Convert all of the post ids to integers
  // std::vector<int64_t> post_ids;
  // for (auto &post_id_str : post_ids_str) {
  //   post_ids.emplace_back(std::stoul(post_id_str));
  // }

  // This goes through and looks for more post ids in mongodb
  // and makes sure not to add duplicates that we already have from redis
  // find in mongodb
  ...
  auto user_timeline_client = user_timeline_client_wrapper->GetClient();
  try {
    user_timeline_client->GetUserTimeline(post_ids, user_id, start, stop);
  } catch (...) {
    _user_timeline_client_pool->Remove(user_timeline_client_wrapper);
    LOG(error) << "Failed to read posts from user-timeline-storage-service";
    throw;
  }
  _user_timeline_client_pool->Keepalive(user_timeline_client_wrapper);

  // Make a vector of the posts given from the post ids
